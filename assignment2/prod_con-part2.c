...
struct myinccountingsem{
    int val;
    sem_t wait;
    sem_t mutex;
};

void myinccountingsem_init(struct myinccountingsem *sem ,int k){
    sem->val =k;
    sem_init(&sem->wait,0,0);
    sem_init(&sem->mutex,0,1);
}

void myinccountingsem_wait(struct myinccountingsem *sem){
    sem_wait(&sem->mutex);
    sem->val--;
    if(sem->val <0){
        sem_post(&sem->mutex);
...
void myinccountingsem_post(struct myinccountingsem *sem){
    sem_wait(&sem->mutex);
    sem->val++;
    if(sem->val<=0) sem_post(&sem->wait);
    sem_post(&sem->mutex);
}
...
struct myinccountingsem full_sem1;
struct myinccountingsem empty_sem1;
...
void *consumer(void *thread_n) {
    ...
    while (i++ < PRODUCER_LOOPS) {
        //sem_wait(&empty_sem);
        myinccountingsem_wait(&empty_sem1);
        ...
        myinccountingsem_post(&full_sem1);
...
int main(int argc, int **argv) {
    ...
    //          0,
    //          0);
    myinccountingsem_init(&full_sem1,SIZE);
    myinccountingsem_init(&empty_sem1,0);
    ...
    sem_destroy(&full_sem1);
    sem_destroy(&empty_sem1);
